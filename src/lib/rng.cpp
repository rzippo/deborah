...
double RNG::nfix(int32_t h, uint32_t i)
{
    ...
    for (;;)
    {
        ...
        // start all over
        h = int32_t(rand_int32());
        i = h & 127;
        if (uint32_t(std::abs(h)) < kn[i])
        {
            return (h * wn[i]);
        }
    }
...
double RNG::efix(uint32_t j, uint32_t i)
{
    for (;;)
    {
        if (i == 0)
...
int32_t RNG::poisson(double mu)
{
    ...
    int32_t pois = -1;
    int32_t k, kflag, big_mu, new_big_mu = 0;
...
int32_t RNG::binomial(double pp, int32_t n)
{
    static double c, fm, npq, p1, p2, p3, p4, qn, xl, xll, xlr, xm, xr;

    static double psave = -1.0;
    static int32_t nsave = -1, m = 0;
    ...
    if (pp != psave || n != nsave)
    {
        ...
        else
        {
            ...
            fm = m;
            npq = np * q;
            p1 = (int32_t) (2.195 * sqrt(npq) - 4.6 * q) + 0.5;
            xm = fm + 0.5;
            xl = xm - p1;
            xr = xm + p1;
...
void RNG::multinom(uint32_t n, const vector<double> &probs, vector<uint32_t> &samp)
{
    samp.resize(probs.size());
    RNG::multinom(n, &probs[0], (uint32_t) probs.size(), &samp[0]);
}

void RNG::multinom(uint32_t size, const double *probs, uint32_t num_probs, uint32_t *samp)
{
    if (num_probs == 0)
    {
        return;
    ...
    for (uint32_t i = 0; i < num_probs - 1; i++)
    {
        if (fixed_probs[i] > 0.)
        {
